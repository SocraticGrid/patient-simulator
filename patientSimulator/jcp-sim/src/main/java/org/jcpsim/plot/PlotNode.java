...
public class PlotNode extends PNode {
  ...
  //-----------------------------------------------------------------------------------
    ...
    public PlotNode(String title, boolean cyclic, Trace[] traces, Double maxX) {
        ...
        for (Trace trace : traces) {
            if (trace.color == null) {
                trace.color = getColor(n);
            }
            if (trace.headColor == null) {
                trace.headColor = headColor;
            }
  ...
  //-----------------------------------------------------------------------------------
    ...
    public synchronized void removeTrace(Trace traceToBeRemoved) {
        this.traces.remove(traceToBeRemoved);
    }
    
    public synchronized void addTrace(Trace traceToBeRemoved) {
        this.traces.add(traceToBeRemoved);
    }
  ...
  //-----------------------------------------------------------------------------------
    ...
    public void paint(PPaintContext paintContext) {
        ...
        for (double i = scaleX.ticmin; i <= maxX; i += scaleX.ticd) {
            ...
            if (lclip.intersectsLine(gridLine)) {
                g2.draw(gridLine);
            }
            n++;
        }
