 ...
 * @author Scott Violet
 */
class DxBufferStrategyPaintManager extends DxRepaintManager.PaintManager {
    //
    ...
    protected void dispose() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                synchronized(DxBufferStrategyPaintManager.this) {
                    while (showing) {
                        try {
                            wait();
                    ...
                    bufferInfos = DxBufferStrategyPaintManager.this.bufferInfos;
                    DxBufferStrategyPaintManager.this.bufferInfos = null;
                }
                dispose(bufferInfos);
            }
        });
  ...
  // it is in paint() above.
    ...
    private void resetDoubleBufferPerWindow() {
        if (bufferInfos != null) {
            dispose(bufferInfos);
            bufferInfos = null;
            repaintManager.setPaintManagerToNull();
        }
    }
      ...
      BufferCapabilities caps;
        ...
        public void windowClosed(WindowEvent e) {
            ...
            synchronized(DxBufferStrategyPaintManager.this) {
                while (showing) {
                    try {
                        DxBufferStrategyPaintManager.this.wait();
                    } catch (InterruptedException ie) {
                    }
                }
