 ...
 * @see JRootPane
 */
class DxSystemEventQueueUtilities
{
  ...
  static EventQueue get(JRootPane rootPane) {
     ...
     * of RunnableCanvas's.
     *
     * @see DxSystemEventQueueUtilities.RunnableCanvas
     */
  ...
  public void run() {
      DxRepaintManager rm = DxRepaintManager.currentManager(component);
        ...
        if (schedule) {
          if (System.getProperty("java.version").startsWith("1.6"))
            SunToolkit.getSystemEventQueueImplPP(appContext).
                postEvent(new InvocationEvent(Toolkit.getDefaultToolkit(), req));
          else  
  ...
  synchronized (classLock) {
     ...
     * to restart.
     *
     * @see DxSystemEventQueueUtilities#restartTimerQueueThread
     */
  ...
  public synchronized void run() {
     ...
     *
     * @see DxSystemEventQueueUtilities#postRunnable
     */
  ...
  if (lock == null) {
     ...
     * to this component.
     *
     * @see DxSystemEventQueueUtilities#processRunnableEvent
     */
